...
#import "PropertyWindowsController.h"


static PropertyWindowsController *_sharedPropertiesWindowController = nil;

@implementation PropertyWindowsController
...
{
	if (!_sharedPropertiesWindowController) {
		_sharedPropertiesWindowController = [[self alloc] initWithWindowNibName:[self nibName]];
	}
	return _sharedPropertiesWindowController;
}
...
- (void)setupToolbar
{
	...
	[self addView:docProperty label:@"Documents" image:[NSImage imageNamed:@"control"]];
	[self addView:sizeProperty label:@"Object Size" image:[NSImage imageNamed:@"genericobject"]];
}
...
- (IBAction)showWindow:(id)sender 
{
	...
	if ([[self window] toolbar] == nil) {
		NSToolbar *toolbar = [[NSToolbar alloc] initWithIdentifier:@"PropertyToolbar"];
...
{	
	...
	if (![newView isEqualTo:oldView]) {		
		...
		[[self window] setTitle:[NSString stringWithFormat:NSLocalizedStringFromTable(@"PropertyTitle",@"Localizable",@"Other"),[[toolbarItems objectForKey:identifier] label]]];
	}
}
